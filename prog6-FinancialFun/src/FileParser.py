...
#Creates a new FileParser object
    def __init__(self,fileName):
        ...
        self.processedFile = self.createList()
    ...
    def groupTOC(self,currentTOC):
        ...
        return groupedTOC
    #Creates a dictionary of the file for easier file recall
    def createList(self):
        file = {}
        for item in self.listTOC:
            if "part" in item.lower():
                file[item] = self.searchParts(item)
            else:
                file[item] = self.searchItems(item)
        return file
    #Searches the file for specific parts
